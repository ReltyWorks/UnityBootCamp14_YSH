...
public class PrefabSpawner : MonoBehaviour {
    ...
    void Start() {
        if (prefab != null) { // 프리팹이 등록 되어있을때만
            spawned = Instantiate(prefab);
            // 생성 코드 Instantiate 인스탄트에이트?
            ...
            //  >> 이 경우 위치 값을 넣으면 회전 값도 꼭 넣어줘야함. Quaternion.identity는 회전하지 않는 원래값을 의미함.
        ...
        } else Debug.LogWarning("병신!"); // 프리팹이 등록되어있지 않았을때
