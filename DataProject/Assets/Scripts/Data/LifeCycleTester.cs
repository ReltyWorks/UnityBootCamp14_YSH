...
Update를 활용해 프레임 별 호출을 순서대로 확인해 봅니다.
...
public class LifeCycleTester : MonoBehaviour {
    ...
    int count_per_frame = 0; // 프레임 단위 호출 카운트
    ...
    // 코루틴 설계
    // yield에 의해 대기 후 싸이클로 돌아오는 과정이 존재하며,
    // 보통 Update의 틈새에 맞춰져 실행됩니다. (Update와 별개라는말)
    IEnumerator CustomCoroutine() {
        ...
        Debug.Log("[Coroutine] 1 초 후 다시 실행됩니다.");
    ...
    void OnDestroy() {
        ...
        // 이 위치에서는 파괴 절차가 진행되고 있기 때문에,
        // 다음과 같은 작업은 불가능하거나 무의미 합니다.
        // gameObject.SetActive(true / false)
        //  >> 진행이 되도 실질적으로 의미가 없음. (상태 변경은 파괴 전 단계에서 처리)
